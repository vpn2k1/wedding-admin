...
export function AdminDashboard() {
  ...
  return (
    <main className="min-h-screen luxe-bg px-4 py-8 text-ink sm:px-5 sm:py-10">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid min-w-0 gap-6 lg:grid-cols-[250px_1fr]">
          <aside className="rounded-[1.25rem] border border-champagne bg-white p-3 shadow-card sm:p-4">
            {[
              ...
              <button
                ...
                className={`mb-2 w-full rounded-xl px-4 py-3 text-left text-sm font-bold transition ${
                  activePanel === value ? 'bg-wine text-white shadow-soft' : 'bg-white text-ink/70 hover:bg-ivory'
                }`}
              >
                {label}
              </button>
          ...
          <section className="min-w-0 rounded-[1.25rem] border border-champagne bg-white p-4 shadow-card sm:p-5">
            ...
            {activePanel === 'album' && (
              <div className="grid gap-6 lg:grid-cols-[360px_1fr]">
                <form ref={uploadFormRef} onSubmit={handleUpload} className="space-y-4 rounded-2xl border border-champagne/80 bg-porcelain p-4 shadow-soft">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3">
                    ...
                    {!isLoadingImages && images.map((image, index) => (
                      <article key={image.id} className="overflow-hidden rounded-2xl border border-champagne/80 bg-porcelain shadow-soft">
                        <label className="group relative block aspect-square cursor-pointer bg-champagne">
                          <WeddingImage src={image.imageUrl} alt={image.title || 'Ảnh album cưới'} fill sizes="(min-width: 768px) 240px, 100vw" className="object-cover transition group-hover:scale-105" />
                          <span className="absolute inset-x-3 bottom-3 z-10 rounded-full bg-white/90 px-3 py-2 text-center text-xs font-bold uppercase tracking-[0.14em] text-wine shadow-card transition group-hover:bg-wine group-hover:text-white">
...
function ImageSettingField({
  ...
  const previewValue = isLoading ? '' : value || fallbackValue;
    ...
    <div className="rounded-2xl border border-champagne bg-ivory/70 p-4 md:col-span-2">
      <div className="grid gap-4 sm:grid-cols-[180px_1fr]">
        <div className="relative aspect-[4/3] overflow-hidden rounded-2xl bg-champagne">
          <WeddingImage src={previewValue} alt={label} fill sizes="180px" className="object-cover" />
        </div>
